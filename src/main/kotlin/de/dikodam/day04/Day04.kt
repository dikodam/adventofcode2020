...
class Day04 : AbstractDay() {
    override fun task1(): String {
        val hackedValidPassportCount = passportStrings
            .filter { passportString -> isPassportAsStringValid(passportString) }
            .count()
        return "$hackedValidPassportCount"
    }

    override fun task2(): String {
        val passportsAsMaps = parsePassportsAsMaps(passportStrings)
        val validPassportCount = passportsAsMaps
            .filter { passportAsMap -> isPassportAsMapValid(passportAsMap) }
            .count()
        return "$validPassportCount"
    }

    fun isPassportAsStringValid(passport: String) =
        sequenceOf("byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid")
            .all { passport.contains(it) }

    fun isPassportAsMapValid(passport: Map<String, String>): Boolean {
        // a passport is valid when all keys are present and their associated values are valid
        try {

            val validations = listOf<Pair<String, (String) -> Boolean>>(
                "byr" to { validateBirthYear(it) },
                "iyr" to { validateIssueYear(it) },
                "eyr" to { validateExpirationYear(it) },
                "hgt" to { validateHeight(it) },
                "hcl" to { validateHairColor(it) },
                "ecl" to { validateEyeColor(it) },
                "pid" to { validatePassportId(it) })

            for ((validationKey, validate) in validations) {
                if (!passport.containsKey(validationKey)) {
                    return false
                }
                if (!validate(passport[validationKey]!!)) {
                    return false
                }
            }
        } catch (e: Exception) {
            println("exception while validating passport: $passport")
            throw e
        }
    ...
    fun validateBirthYear(birthYear: String): Boolean {
        val birthYearOrNull = birthYear.toIntOrNull()
        return if (birthYearOrNull != null) {
            birthYearOrNull in 1920..2002
        } else {
            false
        }
    }
    ...
    fun validateIssueYear(issueYear: String): Boolean {
        val issueYearOrNull = issueYear.toIntOrNull()
        return if (issueYearOrNull != null) {
            issueYearOrNull in 2010..2020
        } else {
            false
        }
    }
    ...
    fun validateExpirationYear(expirationYear: String): Boolean {
        val expirationYearOrNull = expirationYear.toIntOrNull()
        return if (expirationYearOrNull != null) {
            expirationYearOrNull in 2020..2030
        } else {
            false
        }
    }
    ...
    fun validateHairColor(hairColor: String): Boolean {
        return hairColorRegex.matches(hairColor)
    }
    ...
    fun validatePassportId(passportId: String): Boolean {
        return passportIdRegex.matches(passportId)
    }
    ...
    private val passportStrings by lazy {
        parsePassports(input)
    }

    fun parsePassports(input: String): List<String> {
        return input.split("\n\n")
    }

    private val input = """iyr:2015
